...
public:
    ...
    inline void WriteBatch(IneryLevelDBBatch &batch) {
        db.WriteBatch(batch, true);
    }

    template<typename ValueType>
    void WriteBatch(const dbk::PrefixType prefixType, ValueType &value) {
        IneryLevelDBBatch batch;
        const string prefix = dbk::GetKeyPrefix(prefixType);
...
private:
    ...
    mutable IneryLevelDBWrapper db; // // TODO: remove the mutable declare
};

template<int32_t PREFIX_TYPE_VALUE, typename __KeyType, typename __ValueType>
class IneryCompositeKVCache {
public:
...
public:
     ...
     */
    IneryCompositeKVCache(): pBase(nullptr), pDbAccess(nullptr) {};

    IneryCompositeKVCache(IneryCompositeKVCache *pBaseIn): pBase(pBaseIn),
        pDbAccess(nullptr) {
        assert(pBaseIn != nullptr);
    };

    IneryCompositeKVCache(CDBAccess *pDbAccessIn): pBase(nullptr),
    ...
    IneryCompositeKVCache(const IneryCompositeKVCache &other) {
        operator=(other);
    }

    IneryCompositeKVCache& operator=(const IneryCompositeKVCache& other) {
        pBase = other.pBase;
    ...
    void SetBase(IneryCompositeKVCache *pBaseIn) {
    ...
    void RegisterUndoFunc(UndoDataFuncMap &undoDataFuncMap) {
        undoDataFuncMap[GetPrefixType()] = std::bind(&IneryCompositeKVCache::UndoDataList, this, std::placeholders::_1);
    }
    ...
    IneryCompositeKVCache<PREFIX_TYPE, KeyType, ValueType>* GetBasePtr() { return pBase; }
...
private:
    mutable IneryCompositeKVCache<PREFIX_TYPE, KeyType, ValueType> *pBase = nullptr;
...
public:
     ...
     * Default constructor, must use set base to initialize before using.
     */
    InerySimpleKVCache(): pBase(nullptr), pDbAccess(nullptr) {};

    InerySimpleKVCache(InerySimpleKVCache *pBaseIn): pBase(pBaseIn),
        pDbAccess(nullptr) {
        assert(pBaseIn != nullptr);
    }

    InerySimpleKVCache(CDBAccess *pDbAccessIn): pBase(nullptr),
        pDbAccess(pDbAccessIn) {
        assert(pDbAccessIn != nullptr);
    }

    InerySimpleKVCache(const InerySimpleKVCache &other) {
        operator=(other);
    }

    InerySimpleKVCache& operator=(const InerySimpleKVCache& other) {
        pBase = other.pBase;
        pDbAccess = other.pDbAccess;
    ...
    void SetBase(InerySimpleKVCache *pBaseIn) {
        assert(pDbAccess == nullptr);
    ...
    void RegisterUndoFunc(UndoDataFuncMap &undoDataFuncMap) {
        undoDataFuncMap[GetPrefixType()] = std::bind(&InerySimpleKVCache::UndoDataList, this, std::placeholders::_1);
    }
...
private:
    mutable InerySimpleKVCache<PREFIX_TYPE, ValueType> *pBase;
    CDBAccess *pDbAccess;
