...
struct IneryBlockInflatedReward {
    ...
    static const IneryBlockInflatedReward EMPTY;
...
class IneryBlockIndexDB : public IneryLevelDBWrapper {
private:
    IneryBlockIndexDB(const IneryBlockIndexDB &);
    void operator=(const IneryBlockIndexDB &);

public:
    IneryBlockIndexDB(bool fMemory = false, bool fWipe = false) :
        IneryLevelDBWrapper(GetDataDir() / "blocks" / "index", 2 << 20 /* 2MB */, fMemory, fWipe) {}

    // IneryBlockIndexDB(const std::string &name, size_t nCacheSize, bool fMemory = false, bool fWipe = false);

public:
    bool GetBlockIndex(const uint256 &hash, IneryDiskBlockIndex &blockIndex);
    bool WriteBlockIndex(const IneryDiskBlockIndex &blockIndex);
    bool EraseBlockIndex(const uint256 &blockHash);
    bool LoadBlockIndexes();

    bool ReadBlockFileInfo(int32_t nFile, IneryBlockFileInfo &fileinfo);
    bool WriteBlockFileInfo(int32_t nFile, const IneryBlockFileInfo &fileinfo);
};
...
public:
    ...
    IneryBlockDBCache(CDBAccess *pDbAccess):
    ...
    IneryBlockDBCache(IneryBlockDBCache *pBaseIn):
...
public:
    ...
    void SetBaseViewPtr(IneryBlockDBCache *pBaseIn) {
    ...
    bool ReadTxIndex(const uint256 &txid, IneryDiskTxPos &pos);
    bool SetTxIndex(const uint256 &txid, const IneryDiskTxPos &pos);
    bool WriteTxIndexes(const vector<pair<uint256, IneryDiskTxPos> > &list);
    ...
    bool GetBlockInflatedReward(IneryBlockInflatedReward &value);
    bool SetBlockInflatedReward(const IneryBlockInflatedReward &value);
public:
/*  IneryCompositeKVCache      prefixType               key                     value                 variable               */
/*  ----------------   -------------------------   -----------------------  ------------------   ------------------------ */
    // txId -> DiskTxPos
    IneryCompositeKVCache< dbk::TXID_DISKINDEX,         uint256,                  IneryDiskTxPos >          tx_diskpos_cache;
    // flag$name -> bool
    IneryCompositeKVCache< dbk::FLAG,                   string,                   bool>                 flag_cache;

/*  InerySimpleKVCache          prefixType             value           variable           */
/*  -------------------- --------------------   -------------   --------------------- */
    InerySimpleKVCache< dbk::BEST_BLOCKHASH,            uint256>      best_block_hash_cache;    // best blockHash
    InerySimpleKVCache< dbk::LAST_BLOCKFILE,            int32_t>          last_block_file_cache;
    InerySimpleKVCache< dbk::REINDEX,                   bool>         reindex_cache;
    InerySimpleKVCache< dbk::FINALITY_BLOCK,            std::pair<int32_t,uint256>> finality_block_cache;
    InerySimpleKVCache< dbk::BLOCK_INFLATED_REWARD,     IneryBlockInflatedReward>       block_inflated_reward_cache;
};
