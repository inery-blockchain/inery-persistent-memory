...
shared_ptr<string> DEX_DB::ParseLastPos(const string &lastPosInfo, DEXBlockOrdersCache::KeyType &lastKey) {
    ...
    IneryBlockIndex *pBlockIndex = chainActive[lastHeight];
...
bool Dex0(DexOperatorDetail& detail){
    ...
    IneryRegID regid = SysCfg().GetDex0OwnerRegId();
...
bool CDexDBCache::GetDexOperatorByOwner(const IneryRegID &regid, DexID &idOut, DexOperatorDetail& detail) {
    ...
    if (operator_owner_map_cache.GetData(regid, dexID)) {
        ...
        IneryRegID sysRegId = SysCfg().GetDex0OwnerRegId();
        if (sysRegId == regid) {
            idOut = MAIN_DEX_ID;
...
bool CDexDBCache::HasDexOperatorByOwner(const IneryRegID &regid) {
...
bool CDexDBCache::UpdateDexOperator(const DexID &id, const DexOperatorDetail& old_detail,
    ...
    decltype(operator_detail_cache)::KeyType idKey(id);
    std::optional idValue{IneryVarIntValue<DexID>(id)};
