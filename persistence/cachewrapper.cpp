...
CCacheDBManager::CCacheDBManager(bool isReindex, bool isMemory): is_reindex(isReindex), is_memory(isMemory) {
    ...
    pSysParamCache  = new InerySysParamDBCache(pSysParamDb);

    pAccountDb      = CreateDbAccess(DBNameType::ACCOUNT);
    pAccountCache   = new IneryAccountDBCache(pAccountDb);
    ...
    pDelegateCache  = new IneryDelegateDBCache(pDelegateDb);

    pCdpDb          = CreateDbAccess(DBNameType::CDP);
    pCdpCache       = new IneryCdpDBCache(pCdpDb);
    ...
    pBlockIndexDb   = new IneryBlockIndexDB(is_memory, is_reindex);

    pBlockDb        = CreateDbAccess(DBNameType::BLOCK);
    pBlockCache     = new IneryBlockDBCache(pBlockDb);

    pLogDb          = CreateDbAccess(DBNameType::LOG);
    pLogCache       = new IneryLogDBCache(pLogDb);

    pReceiptDb      = CreateDbAccess(DBNameType::RECEIPT);
    pReceiptCache   = new IneryTxReceiptDBCache(pReceiptDb);

    pUtxoDb         = CreateDbAccess(DBNameType::UTXO);
    pUtxoCache      = new IneryTxUTXODBCache(pUtxoDb);

    pAxcDb          = CreateDbAccess(DBNameType::AXC);
    pAxcCache       = new CAxcDBCache(pAxcDb);

    pSysGovernDb    = CreateDbAccess(DBNameType::SYSGOVERN);
    pSysGovernCache = new InerySysGovernDBCache(pSysGovernDb);

    pPriceFeedDb    = CreateDbAccess(DBNameType::PRICEFEED);
    pPriceFeedCache = new IneryPriceFeedCache(pPriceFeedDb);


    // memory-only cache
    pTxCache        = new IneryTxMemCache();
    pPpCache        = new IneryPricePointMemCache();
}
