...
// Batch of changes queued to be written to a IneryLevelDBWrapper
class IneryLevelDBBatch {
    friend class IneryLevelDBWrapper;
...
public:
    IneryLevelDBWrapper(const boost::filesystem::path &path, size_t nCacheSize, bool fMemory = false, bool fWipe = false);
    ~IneryLevelDBWrapper();
    ...
    bool WriteBatch(IneryLevelDBBatch &batch, bool fSync = false);
