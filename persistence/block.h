...
class IneryBlockDBCache;
class IneryDiskBlockPos;
class CNode;
...
public:
    ...
    void SetNull() {
        nVersion       = IneryBlockHeader::CURRENT_VERSION;
...
class IneryBlock : public IneryBlockHeader {
public:
    // network and disk
    ...
    IneryBlock(const IneryBlockHeader &header) {
        SetNull();
        *((IneryBlockHeader *)this) = header;
    }

    IMPLEMENT_SERIALIZE(
        READWRITE(*(IneryBlockHeader *)this);
        READWRITE(vptx);
    )
    ...
    void GetBlockHeader(IneryBlockHeader &header) {
    ...
    const IneryUserID &GetProducerUserID() const;
 ...
 */
class IneryBlockIndex {
public:
    // pointer to the hash of the block, if any. memory is owned by this IneryBlockIndex
    const uint256 *pBlockHash = nullptr;

    // pointer to the index of the predecessor of this block
    IneryBlockIndex *pprev = nullptr;

    // pointer to the index of some further predecessor of this block
    IneryBlockIndex *pskip = nullptr;
    ...
    IneryBlockIndex(const IneryBlock &block) {
    ...
    IneryDiskBlockPos GetBlockPos() const {
        IneryDiskBlockPos ret;
    ...
    IneryDiskBlockPos GetUndoPos() const {
        IneryDiskBlockPos ret;
    ...
    int64_t GetMedianTimePast() const {
        ...
        const IneryBlockIndex *pIndex = this;
     ...
     */
    static bool IsSuperMajority(int32_t minVersion, const IneryBlockIndex *pstart,
                                uint32_t nRequired, uint32_t nToCheck);

    string ToString() const {
        return strprintf("IneryBlockIndex(pprev=%p, height=%d)", pprev, height);
    }
    ...
    IneryBlockIndex *GetAncestor(int32_t heightIn);
    const IneryBlockIndex *GetAncestor(int32_t heightIn) const;
};

struct IneryBlockIndexWorkComparator {
    bool operator()(IneryBlockIndex *pa, IneryBlockIndex *pb) const {
...
class IneryDiskBlockIndex : public IneryBlockIndex {
public:
    uint256 hashPrev;
    uint256 merkleRootHash;
    uint32_t nBits = 0;
    ...
    // explicit IneryDiskBlockIndex(IneryBlockIndex *pIndex) : IneryBlockIndex(*pIndex) {
    //     hashPrev = (pprev ? pprev->GetBlockHash() : uint256());
    // }

    IneryDiskBlockIndex(IneryBlockIndex *pIndex, const IneryBlock &block, const IneryRegID &bpRegid): IneryBlockIndex(*pIndex) {
        hashPrev       = pprev ? pprev->GetBlockHash() : uint256();
 ...
 */
struct IneryBlockLocator {
    vector<uint256> vHave;

    IneryBlockLocator() {}
    IneryBlockLocator(const vector<uint256> &vHaveIn) { vHave = vHaveIn; }
...
bool WriteBlockToDisk(IneryBlock &block, IneryDiskBlockPos &pos);
bool ReadBlockFromDisk(const IneryDiskBlockPos &pos, IneryBlock &block);
bool ReadBlockFromDisk(const IneryBlockIndex *pIndex, IneryBlock &block);


bool ReadBaseTxFromDisk(const IneryTxCord txCord, std::shared_ptr<CBaseTx> &pTx);

template<typename TxType>
bool ReadTxFromDisk(const IneryTxCord txCord, std::shared_ptr<TxType> &pTx) {
...
bool GetBlockHeader(IneryBlockIndex *pBlockIndex, IneryBlockHeader &header);
